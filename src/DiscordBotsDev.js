const DBDRequest = require('./DBDRequest.js');
/**
...
module.exports = class DiscordBotsDev {
    ...
    constructor(token, clientID, ownerID) {
        ...
        const request = new DBDRequest(this.baseAPIUrl);
        ...
        /**
         * DBDRequest Client for creating requests
         * @private
         */
...
async function tokenValidator(token, request) { //eslint-disable-line no-unused-vars
    var response = await request.post('tokenValidator', { token: token });
...
async function fetchToken(token, clientID, ownerID, request) { //eslint-disable-line
    var response = await request.post('fetchToken', { token: token });
    var body = await response.body;
